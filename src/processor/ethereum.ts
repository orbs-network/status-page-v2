...
export async function getEthereumStatus(stakingRewardsAddress:string, bootstrapRewardsAddress: string, config: Configuration): Promise<EthereumStatus> {
    const web3 = getWeb3(config.EthereumEndpoint);
    const stakeRewardContract = getContract(web3, protocolWalletAbi, stakingRewardsAddress);
    const bootstrapRewardContract = getContract(web3, protocolWalletAbi, bootstrapRewardsAddress);
